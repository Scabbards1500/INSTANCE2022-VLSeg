...
class DataAug3D(object):
    ...
    def __ImageMaskTranform(self, images_path, index, number, maxvalue=255):
        ...
        # Get the original file names from the paths
        image_filename = os.path.splitext(os.path.basename(images_path[0]))[0]
        mask_filename = os.path.splitext(os.path.basename(images_path[1]))[0]
        ...
        for batchx, batchy in self.__datagen.flow(srcimage, srcmask):
            ...
            # npy_path1 = os.path.join(self.aug_path, 'data', f'{image_filename}_{i}.npy')
            # npy_path2 = os.path.join(self.aug_path, 'label', f'{mask_filename}_{i}.npy')

            npy_path1 = self.aug_path + '\\data\\' + image_filename + '_' + str(i) + ".npy"
            batch1 = batch1.reshape([srcimages.shape[0], srcimages.shape[1], srcimages.shape[2]])
            np.save(npy_path1, batch1)

            npy_path2 = self.aug_path + '\\label\\' + mask_filename + '_' + str(i) + ".npy"
