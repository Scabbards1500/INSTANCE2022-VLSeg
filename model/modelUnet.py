...
class BinaryUNet3dModel(object):
    ...
    def __init__(self, image_depth, image_height, image_width, image_channel,bert_type, numclass, batch_size,
    ...
    def _dataloder(self, images, labels, text, bert_type, shuffle=False):
        """"
        train dataset shuffle is true,validation is false
        """
        dataset = datasetModelSegwithnpy(images, labels, text, bert_type,
                                         targetsize=(self.image_channel, self.image_depth, self.image_height, self.image_width))
        # fow window num_workers is only zero,for linux num_workers can not zero
        # num_cpu = multiprocessing.cpu_count()
        num_cpu = 0
        dataloader = DataLoader(dataset, shuffle=shuffle, batch_size=self.batch_size, num_workers=num_cpu,
    ...
    def trainprocess(self, trainimage, trainmask, validationimage, validationmask, traintext, valtext, model_dir, bert_dir, epochs=50, lr=1e-3,
                     showwind=[8, 8]):
        print("[INFO] training the network...")
        Path(model_dir).mkdir(parents=True, exist_ok=True)
        ...
        # lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(opt, 'min', patience=2, verbose=True)
        # 2、load data train and validation dataset
        train_loader = self._dataloder(trainimage, trainmask,traintext, bert_type, True)
        val_loader = self._dataloder(validationimage, validationmask,valtext, bert_type, True)
        # 3、initialize a dictionary to store training history
        H = {"train_loss": [], "train_accuracy": [], "valdation_loss": [], "valdation_accuracy": []}
        # 4、start loop training wiht epochs times
        startTime = time.time()
        ...
        for e in tqdm(range(epochs)):
            ...
            for batch in train_loader:
                ...
                # perform a forward pass and calculate the training loss and accu
                pred_logit, pred = self.model(x,t) # pred_logit is the output of the model without sigmoid, pred is the output of the model with sigmoid
                loss = lossFunc(pred_logit, y)
                accu = self._accuracy_function(self.accuracyname, pred, y)
                # save_images
                if trainshow:
...
class BinaryUNet3dModel_ori(object):
    """
    Unet3d with binary class,should rewrite the dataset class and inference fucntion
    """
    ...
    def __init__(self, image_depth, image_height, image_width, image_channel, numclass, batch_size,
        ...
        self.model = UNet3d_ori(self.image_channel, self.numclass)
        self.model.to(device=self.device)
    ...
    def _dataloder(self, images, labels, shuffle=False):
        ...
        train dataset shuffle is true,validation is false
        """
        dataset = datasetModelSegwithnpy_ori(images, labels,
                                         targetsize=(
